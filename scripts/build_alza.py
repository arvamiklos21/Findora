# category_assign.py
#
# Findora – kategorizáló motor (ALZA-specifikus logikával)
#
# Ez a modul egyetlen publikus függvényt exportál:
#
#   assign_category(fields: dict) -> str
#
# A build_alza.py így hívja:
#
#   fields_for_cat = {
#       "title": title or "",
...
#
# A visszatérési érték egy Findora főkategória ID:
#   "elektronika",
#   "haztartasi_gepek",
#   "szamitastechnika",
#   "mobiltelefon",
...
# ===== ROOT → alap Findora főkategória =====
def _base_category_from_root(root: str) -> Optional[str]:
    """
    ...
    # TV / audio / video / foto
    if root.startswith("tv, foto, audio") or root.startswith(
        "tv, foto, audio-video"
    ) or "audio, video" in root or "audio-video" in root:
        return "elektronika"
    ...
    if (
        root.startswith("haztartasi kisgep")
        or root.startswith("haztartasi nagyg")
...
# ===== Speciális finomhangolások =====
def _refine_for_special_roots(
    base_cat: Optional[str],
    root: str,
    parts_norm: list,
    full_norm: str,
) -> Optional[str]:
...
# ===== Kulcsszavas fallback, ha a root nem ad kategóriát =====
def _keyword_fallback(full_norm: str) -> Optional[str]:
    """
    Ha a root alapján nem tudtunk dönteni, kulcsszavas fallback.
    Csak akkor fut, ha base_cat None.
    """
    # Elektronika
    if any(
        kw in full_norm
    ...
    # Könyv
    if any(kw in full_norm for kw in ["konyv", "regeny", "tankonyv", "kotta"]):
        return "konyv"
...
# ===== ALZA-specifikus kategorizáló belső függvény =====
def _assign_category_alza(category_path: str, title: str, description: str) -> str:
    """
    ALZA-termék Findora főkategória hozzárendelése.
    """
...
def assign_category(fields: Dict[str, Any]) -> str:
    """
    Publikus Findora kategorizáló belépési pont.

    Vár egy dict-et (fields), pl.:
    ...
    Most ALZA-specifikus logikát futtatunk a category_path + title + description alapján.
    """
    ...
    return _assign_category_alza(str(category_path), str(title), str(desc))
