...
#
# KozmetikaOtthon feed → Findora JSON oldalak
#
# Kategorizálás:
#   - MINDEN termék a "szepseg" fő kategóriába megy
#   - de MIND a 25 Findora kategória mappa létrejön (akkor is, ha üres)
#
# Kimenet:
#   docs/feeds/kozmetikaotthon/meta.json, page-0001.json        (globál)
#   docs/feeds/kozmetikaotthon/<findora_cat>/meta.json, page-...json (kategóriák)
#   docs/feeds/kozmetikaotthon/akcio/meta.json, page-...json    (akciós blokk, discount >= 10%)
#
# FONTOS:
#   - Ha a feed URL hibás, timeoutol, vagy nincs beállítva:
#       → üres "items" lista
#       → DE akkor is létrejön:
#           - globál meta + page-0001.json
#           - mind a 25 kategória meta + page-0001.json
#           - akcio meta + page-0001.json
...
def main():
    ...
    if not FEED_URL:
        print("⚠️ FEED_KOZMETIKAOTTHON_URL nincs beállítva → üres skeleton készül.")
    else:
        try:
            ...
            print(f"⚠️ KozmetikaOtthon feed hiba vagy időtúllépés → üres skeleton. Részletek: {e}")
            items = []

    # NORMALIZÁLÁS – MINDEN termék "szepseg" fő kategóriába
    rows = []
    for it in items:
        pid = it["id"]
        title = it["title"]
    ...
    # 1) Globál feed (lehet üres is, de page-0001.json akkor is lesz)
    paginate_and_write(
        OUT_DIR,
        rows,
        PAGE_SIZE_GLOBAL,
    ...
    # 2) KATEGÓRIA FEED-EK – MIND a 25 slug kap mappát + meta + page-0001
    ...
    # 3) AKCIÓS BLOKK (discount >= 10%) – akkor is létrejön, ha üres
    akcios_items = [
        row for row in rows
